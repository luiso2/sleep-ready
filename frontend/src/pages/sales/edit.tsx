...
export const SaleEdit: React.FC = () => {
  const { formProps, saveButtonProps, form, queryResult } = useForm<ISale>();
  const saleData = queryResult?.data?.data;
  const [saleType, setSaleType] = useState<string>(saleData?.type || "product");
